...
public class SGIPClient {
	 ...
	 * @throws Exception 
	 */
	public static boolean sendMsg(String userNumber, String content)
	{
		...
		try {
			 ...
			 resultFlag =startCommunication(listUserNumber, content);
	...
	private static boolean startCommunication(List<String> listUserNumber,
					...
					} else
					{
						...
						for(int i = 0; i < multi; i++)
						{
							...
							if(content.length()>70){
								...
								Vector<byte[]> splitContent = SplitContent(content.getBytes("UTF-16")); 
								for (int ii = 0; ii < splitContent.size(); ii++) {
									msg = SGIPFactory.getSGIPMsg(SGIPConstant.SGIP_SUBMIT);
									
									msg.setLongUserNumbers(tempUserNumbers, addContentHeader(splitContent.get(ii),
											splitContent.size(), ii + 1));
	...
	{
		List<String> listUserNumber = new ArrayList<String>();
		listUserNumber.add("18584360524");
		//sendMsg(listUserNumber, "just test test-NIO方式 ",true);
		//sendMsg(listUserNumber, "just test test-SOCKET方式 ",false);
	...
	public static void sendTestMsg() throws Exception
	{
		List<String> listUserNumber = new ArrayList<String>();
		listUserNumber.add("18584360524");
		sendMsg(listUserNumber, "just test test短信测试");
	}
}